...
const NewClock: React.FC<NavigationInjectedProps> = ({ navigation }) => {
	const refreshClocks = navigation.state.params.refreshClocks;
	const [selectMode, setSelectMode] = useState<'hour' | 'minute'>('hour');
		...
		<View style={styles.container}>
			<View style={styles.timeContainer}>
				<TouchableOpacity onPress={() => setSelectMode('hour')}>
					<Text style={{ ...styles.selectedTimeText, color: selectMode === 'hour' ? colors.accentColor : colors.textPrimaryColor }}>
						{String(hour).padStart(2, '0')}
					</Text>
				</TouchableOpacity>
				<Text style={styles.selectedTimeText}>:</Text>
				<TouchableOpacity onPress={() => setSelectMode('minute')}>
					<Text style={{ ...styles.selectedTimeText, color: selectMode === 'minute' ? colors.accentColor : colors.textPrimaryColor }}>
						{String(minute).padStart(2, '0')}
					</Text>
				</TouchableOpacity>
			</View>
			<HourPicker mode={selectMode} setHour={setHour} setMinute={setMinute} setSelectMode={setSelectMode} />
